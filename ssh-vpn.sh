...
# install
apt-get --reinstall --fix-missing install -y bzip2 gzip coreutils wget screen rsyslog iftop htop net-tools
apt-get install -y zip unzip wget net-tools curl nano sed screen gnupg gnupg1 bc apt-transport-https build-essential dirmngr;
apt-get -y install libxml-parser-perl;
apt -y install neofetch git
echo "clear" >> .profile
echo "neofetch" >> .profile
echo "echo AutoscriptVPS By Horasss" >> .profile
...
wget -O /etc/issue.net "https://sc.vpnstores.net/bannerssh.conf"
echo "Banner /etc/issue.net" >>/etc/ssh/sshd_config
sed -i 's@DROPBEAR_BANNER=""@DROPBEAR_BANNER="/etc/issue.net"@g' /etc/default/dropbear

#install bbr dan optimasi kernel
wget https://sc.vpnstores.net/bbr.sh && chmod +x bbr.sh && ./bbr.sh
